...
Purpose of the this hands-on training is to give understanding to take a manual snapshot from RDS DB instance, restore RDS DB instance from this snapshot, and also restore RDS DB instance from a point in time. We'll use the RDS created in former session.(Hands-on DB-01)
...
Security Group
- Select Client_Sec_Group
```

- Connect Ubuntu Instance with SSH. ---> ssh -i serdar.pem ubuntu@<IP adress>
...
- Login back to the RDS MySQL DB instance (`RDS-mysql`) as `admin` using the password defined with SSL pass: `Pp123456789`.
...
- Restore database from manual snapshot as new DB instance and name it as `restored-from-manuel-snapshot`.
  ...
  - Settings:
    DB Instance identifier : restored-from-manuel-snapshot
  ...
  - Additional connectivity configuration:
    Subnet Group : Default
    Public Accessible: Yes ****Please change it*****
    Existing VPC security groups: DatabaseSecGrb ****please change it*****
  ...
  - Additional configuration:
    Keep it as is ---> As you see, there is no database section, because my database has already ready. 
  ```

- This time we are going to connect as `admin` using the password defined `Pp123456789` to the newly RDS Instance named `restored-from-manuel-snapshot` that is created from snapshot.

```bash
mysql -h [***restored-from-manuel-snapshot RDS endpoint] -u admin -p
```
...
- Show that deleted records of employees are back in `restored-from-manuel-snapshot`

```sql
SELECT * FROM employees;
```
- Exit from`restored-from-manuel-snapshot` database
...
- Log into the RDS instance (`restored-from-point-in-time-RDS`) as `admin` using the password defined `Pp123456789`
